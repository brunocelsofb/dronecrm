...
// Lista de migraÃÂ§ÃÂµes consolidadas Ã¢ÂÂ cada uma ÃÂ© idempotente (add column if not exists, create table if not exists).
...
const MIGRATIONS: Array<{ id: string; description: string; sqls: string[] }> = [
  ...
  },
  {
    ...
    description: 'Tabela de configuraÃÂ§ÃÂ£o de campos por funil',
    sqls: [
  ...
  },
  {
    ...
    description: 'Tabela de mediÃÂ§ÃÂµes mensais',
    sqls: [
...
export async function GET() {
  return NextResponse.json({
    message: 'API de migraÃÂ§ÃÂµes ORBIS CRM',
...
export async function POST(request: Request) {
  ...
  if (key !== (process.env.MIGRATION_SECRET_KEY ?? 'orbis-migrate-2026')) {
    return NextResponse.json({ error: 'Chave invÃÂ¡lida' }, { status: 401 })
  }
  ...
  for (const migration of MIGRATIONS) {
    ...
    for (const sql of migration.sqls) {
      ...
      const { error } = await (supabase as any).rpc('exec_migration_sql', { p_sql: sql }).catch(async (e: any) => {
        // Se o RPC nÃÂ£o existir, tenta via from().select() que o admin client permite
        return { error: { message: `RPC nÃÂ£o disponÃÂ­vel: ${e.message}. Rode o SQL manualmente.` } }
      })
      if (error) {
  ...
  return NextResponse.json({ results, tip: 'Se aparecer "RPC nÃÂ£o disponÃÂ­vel", copie os SQLs da lista abaixo e rode no SQL Editor do Supabase.', migrations: MIGRATIONS.map(m => ({ id: m.id, sqls: m.sqls })) })
}
